...
app.post("/register", (req, res) => {
  ...
  return res.json({
    ...
    UserID: userID,
    temporaryToken: mytoken,
    tokenValidity: "20 minutes",
  });
});
...
app.post("/blockdata", (req, res) => {
  ...
  return res.send(
    "Your Transaction is added to Queue. Transaction ID is: " +
      auto_id +
...
function getPendingTransactions(userid, resp) {
  ...
  resp.json({
    allUsersTransactions_pending: transactionList.length,
...
function postBlockChain(block_chain_json, resp) {
  ...
  for (let i = 0; i < block_chain_json.length; i++) {
    ...
    if (
      !bchain.hash ||
      typeof bchain.prevHash == "undefined" ||
      !bchain.timestamp ||
      typeof bchain.nonce == "undefined"
    ) {
      return resp.send("Error: Invalid Blockchain");
    }
  ...
  return new Promise((resolve, reject) => {
    const worker = new Worker("./worker.js", {
      workerData: {
        ...
        transactionData: transactionList[0].data,
